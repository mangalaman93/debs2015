...
public class Source2 implements StatelessOperator {
...
//						values[j] = new Integer(((int)longVal)	); break;
	...
	public void setUp(URL url) throws IOException {
		...
		Thread threadForReading = new Thread(new DiskReader(queue));
		threadForReading.start();
		tuple_queue = new ArrayBlockingQueue<DataTuple>(blockSize, false);
		//dp = new DataParser(queue, tuple_queue); 
		//Thread threadForParsing = new Thread(dp);
		//threadForParsing.start();
...
class DiskReader implements Runnable {
	...
	public DiskReader(BlockingQueue<String> queue) {
		System.out.println("DiskReader instantiated.");
		this.ip_queue = queue;
		URL url;
		try {   
			url = new URL(filePath);
	...
	@Override
	public void run() {
		System.out.println("DiskReader running.");
		while (true)
			try {   
				String line = reader.readLine();
				ip_queue.put(line);
...
class DataParser implements Runnable {
	...
	public DataParser(BlockingQueue<String> ip_queue, BlockingQueue<DataTuple> op_queue) {
